...
export class PatientsService {
  ...
  public async getPatientsList(page: number, limit: number, name?: string, phone?: string, chartNumber?: string): Promise<PatientListResponse> {
    try {
      ...
      const [data, total] = await this.patientRepository.findAndCountPatients(
        page, 
        limit, 
        name, 
        phone, 
      ...
      return { total, page, count: limit, data, totalPages };
  ...
  private async savePatients(mergedPatients: Patient[]): Promise<void> {
    ...
    this.logger.log(`[savePatients] 환자 저장 start: ${mergedPatients.length}개`);
    ...
    try {
      ...
      const existingPatients = await this.patientRepository.findByNameAndPhone(patientNames, patientPhones);
      ...
      for (const patient of mergedPatients) {
        ...
        if (patient.chart && patient.chart !== '') {
          ...
          if (dbPatientEmptyChart) {
            ...
            this.updatePatientFields(dbPatientEmptyChart, patient);
            ...
            if (dbPatient) {
              ...
              this.updatePatientFields(dbPatient, patient);
          ...
          if (hasChartPatient) {
            ...
            this.updatePatientFields(hasChartPatient, patient);
      ...
      if (patientsToInsert.length > 0) {
        await this.patientRepository.bulkInsert(patientsToInsert);
        this.logger.log(`[savePatients] 환자 삽입 완료: ${patientsToInsert.length}개`);
      }
      
      if (patientsToUpdate.length > 0) {
        const updateCount = await this.patientRepository.updatePatientsInChunks(patientsToUpdate);
        this.logger.log(`[savePatients] 환자 업데이트 완료: ${updateCount}개`);
      }

      //끝
      const endTime = Date.now();
      this.logger.log(`[savePatients] DB 저장 소요 시간: ${endTime - startTime}ms`);
  ...
  private updatePatientFields(dbPatient: Patient, newData: Patient): void {
    /**
     * dbPatient의 필드를 업데이트하는 헬퍼 함수
     */
    if (newData.rrn && newData.rrn !== '') {
      dbPatient.rrn = newData.rrn;
    }
    if (newData.address && newData.address !== '') {
      dbPatient.address = newData.address;
    }
    if (newData.memo && newData.memo !== '') {
      dbPatient.memo = newData.memo;
    }
  }
